...
#
# Steps:
#   1. Build cmd/markdown-ref-alfred as a universal (amd64+arm64)
...
GOOS=darwin GOARCH=amd64 go build -o "${BUILD_DIR}/markdown-ref-alfred-amd64" ./cmd/markdown-ref-alfred
GOOS=darwin GOARCH=arm64 go build -o "${BUILD_DIR}/markdown-ref-alfred-arm64" ./cmd/markdown-ref-alfred
lipo -create -output "${BUILD_DIR}/markdown-ref-alfred" \
  "${BUILD_DIR}/markdown-ref-alfred-amd64" \
  "${BUILD_DIR}/markdown-ref-alfred-arm64"
rm "${BUILD_DIR}/markdown-ref-alfred-amd64" "${BUILD_DIR}/markdown-ref-alfred-arm64"
chmod +x "${BUILD_DIR}/markdown-ref-alfred"
lipo -info "${BUILD_DIR}/markdown-ref-alfred"

if [ -n "${CODESIGN_IDENTITY:-}" ]; then
  echo "→ Signing entrypoint binary (${CODESIGN_IDENTITY})"
  codesign --force --options runtime --timestamp --sign "$CODESIGN_IDENTITY" \
    "${BUILD_DIR}/markdown-ref-alfred"
  codesign --verify --strict --verbose=2 "${BUILD_DIR}/markdown-ref-alfred"

  if [ -n "${NOTARY_KEY_ID:-}" ]; then
    "${REPO_ROOT}/scripts/notarize-binary.sh" "${BUILD_DIR}/markdown-ref-alfred"
  fi
else
